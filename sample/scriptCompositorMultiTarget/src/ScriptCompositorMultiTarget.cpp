#include "ScriptCompositorMultiTarget.hpp"

#include <iostream>

ScriptCompositorMultiTarget::ScriptCompositorMultiTarget() :
    FrameListener(),
...
void ScriptCompositorMultiTarget::start()
{
    ...
    m_resourceLoader.initialise("ScriptCompositorMultiTarget");

	createObject();

    ::Ogre::CompositorManager::getSingleton().addCompositor(m_viewport, "script_compositor_multi_target_C");
    ::Ogre::CompositorManager::getSingleton().setCompositorEnabled(m_viewport, "script_compositor_multi_target_C", true);
}

void ScriptCompositorMultiTarget::stop()
{
	destroyObject();
...
void ScriptCompositorMultiTarget::run()
{
    int i=0;
    while (++i < 10000)
    {
...
void ScriptCompositorMultiTarget::loadScene()
{
...
void ScriptCompositorMultiTarget::unloadScene()
{
...
void ScriptCompositorMultiTarget::createObject()
{
    // Light
	//----------------------------------
	{
	...
	//----------------------------------
	{
        ::Ogre::Entity* cube = m_sceneManager->createEntity("OGRENGINE_Cube", "cube.mesh", "ScriptCompositorMultiTarget");
        cube->setMaterialName("script_material_multi_target_M");
...
void ScriptCompositorMultiTarget::destroyObject()
{
    // Cube
    //----------------------------------
    {
...
bool ScriptCompositorMultiTarget::frameStarted(const Ogre::FrameEvent&)
{
	return true;
}

bool ScriptCompositorMultiTarget::frameEnded(const Ogre::FrameEvent&)
{
	return true;
}

bool ScriptCompositorMultiTarget::frameRenderingQueued(const Ogre::FrameEvent&)
{
    // Rotate the cube
	//---------------------------------
	{
