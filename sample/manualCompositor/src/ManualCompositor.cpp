...
ManualCompositor::ManualCompositor() :
    FrameListener(),
    ...
    m_compositor("manual_compositor_C")
{
...
void ManualCompositor::start()
{
    ...
    m_resourceLoader.initialise("ManualCompositor");

    m_compositor.load();

	createObject();

    ::Ogre::CompositorManager::getSingleton().addCompositor(m_viewport, "manual_compositor_C");
    ::Ogre::CompositorManager::getSingleton().setCompositorEnabled(m_viewport, "manual_compositor_C", true);
}

void ManualCompositor::stop()
{
	destroyObject();
...
void ManualCompositor::loadScene()
{
...
void ManualCompositor::unloadScene()
{
...
void ManualCompositor::createObject()
{
    // Light
	//----------------------------------
	{
	...
	//----------------------------------
	{
        ::Ogre::Entity* cube = m_sceneManager->createEntity("OGRENGINE_Cube", "cube.mesh", "ManualCompositor");
...
void ManualCompositor::destroyObject()
{
    // Cube
    //----------------------------------
    {
...
bool ManualCompositor::frameStarted(const Ogre::FrameEvent&)
{
	return true;
}

bool ManualCompositor::frameEnded(const Ogre::FrameEvent&)
{
	return true;
}

bool ManualCompositor::frameRenderingQueued(const Ogre::FrameEvent&)
{
    // Rotate the cube
	//---------------------------------
	{
